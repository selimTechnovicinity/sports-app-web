...
interface CoachAdditionModalProps {
  open: boolean;
  onClose: () => void;
}

const CoachAdditionModal = ({ open, onClose }: CoachAdditionModalProps) => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      await API.post(`/teams/${data.team_id}/coach`, data);
      showToast("Coach added successfully", "success", "Success");
      reset();
      onClose();
    } catch (error: any) {
      showToast(
        error.response?.data?.message || "Failed to add Coach",
        "error",
        "Error"
      );
    } finally {
...
export default CoachAdditionModal;
